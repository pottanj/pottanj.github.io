...
function frame(){
  ...
  depths.forEach(el=>{const d=Number(el.dataset.depth);el.style.setProperty('--mx',`${cx*d*75}px`);el.style.setProperty('--my',`${cy*d*42}px`)});
  if(Math.abs(tx-cx)>.01||Math.abs(ty-cy)>.01)pointerFrame=requestAnimationFrame(frame);
}
window.addEventListener('pointermove',e=>{
  tx=(e.clientX/innerWidth-.5)*2;ty=(e.clientY/innerHeight-.5)*2;
  cursor.style.left=e.clientX+'px';cursor.style.top=e.clientY+'px';
  if(!pointerFrame&&!reducedMotion.matches)pointerFrame=requestAnimationFrame(frame);
},{passive:true});
...
const aetherIframe=aetherCase?.querySelector('iframe[data-src]');
const loadAetherFilm=()=>{if(aetherIframe&&!aetherIframe.src)aetherIframe.src=aetherIframe.dataset.src};
const stopAetherFilm=()=>{if(aetherIframe?.src){aetherIframe.removeAttribute('src')}};
function setAether(open,{move=true}={}){
  ...
  if(open)loadAetherFilm();else stopAetherFilm();
...
const spotifyIframe=document.querySelector('.spotify-player iframe[data-src]');
if(spotifyIframe){
  ...
  spotifyObserver.observe(spotifyIframe);
}
...
projectRows.forEach(row=>{
  ...
  if(panel instanceof HTMLDetailsElement){
    ...
    if(content&&!content.querySelector('.case-expanded-head')){
      ...
      const caseKind=rowMeta?.querySelector('.project-kind')?.textContent.trim()||'';
      head.innerHTML=`<p>${row.querySelector('.project-no')?.textContent||''} / UX CASE</p><div><h2>${row.querySelector('h3')?.textContent||''}</h2><span>${baseMeta}${caseKind?`<small> · <b>${caseKind}</b></small>`:''}</span><p class="case-open-intro">${row.querySelector('.project-question')?.textContent||''}</p></div>`;
...
function closeProjectPanels(except=null){
  projectRows.forEach(row=>{
    ...
    if(panel===aetherCase)stopAetherFilm();
  });
}
workSection?.addEventListener('click',event=>{
  ...
  if(panel===aetherCase){if(!isOpen)loadAetherFilm();else stopAetherFilm();}
...
// Copy added by the expanded case views. Keep this dictionary next to the
// language layer so newly opened projects never fall back to mixed Swedish.
Object.assign(englishCopy,{
  'JAG BYGGER':'I BUILD','UPPLEVELSER':'EXPERIENCES','Praktiskt tänkande, mänsklig förståelse och digital form.':'Practical thinking, human understanding and digital craft.','SCROLLA FÖR ATT UTFORSKA ↓':'SCROLL TO EXPLORE ↓',
  'Från fråga till lösning.':'From question to solution.','Case inom research, UX/UI, tjänstedesign, speldesign och konceptutveckling.':'Cases across research, UX/UI, service design, game design and concept development.','TJÄNSTEDESIGN':'SERVICE DESIGN','UTVALT MATERIAL FRÅN PROJEKTET':'SELECTED PROJECT MATERIAL','VISUELL TOLKNING AV PROJEKTIDÉN':'VISUAL INTERPRETATION OF THE PROJECT IDEA',
  'SAMMANHANG':'CONTEXT','Så tog vi oss an problemet.':'How we approached the problem.','Vad riktningen byggde på.':'What shaped the direction.','Vad arbetet ledde fram till.':'What the work achieved.','Det jag tar vidare.':'What I take forward.','04 / LÄRDOM':'04 / LEARNING','03 / RESULTAT':'03 / OUTCOME','FOKUS':'FOCUS','UX VÄRDE':'UX VALUE','VÄNSTER':'LEFT','HÖGER':'RIGHT',
  'Ett skarpt UX och UI uppdrag för AcadeMedia Academy, en lärplattform för medarbetare inom flera utbildningsverksamheter.':'A real UX and UI project for AcadeMedia Academy, a learning platform for employees across several education organisations.','Startsidan upplevdes som en vägg av text. Användaren behövde snabbare förstå vad som var relevant, pågående och nästa steg.':'The home page felt like a wall of text. Users needed to understand more quickly what was relevant, in progress and next.','Vi kartlade informationsarkitekturen, byggde tidiga flöden och observerade hur användare klickade och orienterade sig i flera prototypversioner.':'We mapped the information architecture, built early flows and observed how users clicked and navigated across several prototype versions.','Personligt innehåll, utbildningsstatus och tydliga vägar till katalogen prioriterades. Hierarki och färg användes för att skilja handling från information.':'Personal content, course status and clear routes to the catalogue were prioritised. Hierarchy and colour separated actions from information.','Den slutliga riktningen gjorde lärandet mer överblickbart, personligt och motiverande utan att tappa AcadeMedias identitet.':'The final direction made learning easier to grasp, more personal and more motivating without losing AcadeMedia’s identity.','När innehållet är omfattande måste gränssnittet först svara på frågan: vad är viktigast för mig just nu?':'When content is extensive, the interface must first answer: what matters most to me right now?',
  'Ett skarpt UX uppdrag för Duro med fokus på deras digitala tapetupplevelse och vägen från inspiration till köp.':'A real UX project for Duro focused on its digital wallpaper experience and the journey from inspiration to purchase.','Det stora sortimentet var svårt att orientera sig i. Sök, filter, guider och butikshittare skapade osäkerhet och granskningen visade även tillgänglighetsproblem.':'The large range was difficult to navigate. Search, filters, guides and the store locator caused uncertainty, and the review also revealed accessibility issues.','Vi kombinerade manuell granskning, enkät, användartester och iterativa Figma prototyper. Tre testomgångar gjorde det möjligt att pröva både helhet och detaljer.':'We combined manual review, a survey, user testing and iterative Figma prototypes. Three test rounds let us evaluate both the overall experience and its details.','Sökförslag förtydligades, filtreringen stabiliserades och guider lyftes fram. Favoritfunktionen och butikshittaren fick tydligare återkoppling.':'Search suggestions became clearer, filtering more stable and guides more prominent. Favourites and the store locator received clearer feedback.','Resultatet blev en mer begriplig riktning för sök, inspiration och köp, förankrad i observerade användarbehov och WCAG.':'The result was a clearer direction for search, inspiration and purchase, grounded in observed user needs and WCAG.','Tillgänglighet och affärsnytta blir starkare när de behandlas som samma designproblem, inte som separata lager.':'Accessibility and business value grow stronger when treated as one design problem rather than separate layers.',
  'Ett skolcase om hur Kommunals befintliga webbupplevelse kan ge tydligare stöd i medlemsärenden och självskattning.':'A student case exploring how Kommunal’s existing website could provide clearer support for membership matters and self-assessment.','Viktiga funktioner låg djupt i navigationen och självskattningen saknade tillräcklig orientering, återkoppling och överblick.':'Important functions were buried in the navigation, while the self-assessment lacked sufficient orientation, feedback and overview.','Befintliga sidor och flöden kartlades. Därefter togs interaktionsflöden och UI iterationer fram i Figma och prövades i användartest.':'Existing pages and flows were mapped. Interaction flows and UI iterations were then created in Figma and evaluated through user testing.','Centrala ingångar lyftes fram. Självskattningen fick skala, stegindikator, brödsmulor och mer skannbara resultat.':'Key entry points were surfaced. The self-assessment gained a scale, progress indicator, breadcrumbs and more scannable results.','Förslaget gav en tydligare väg genom både webbplats och självskattning, samtidigt som Kommunals visuella igenkänning bevarades.':'The proposal created a clearer path through both the website and self-assessment while preserving Kommunal’s visual identity.','Testet visade att rätt navigation måste lösas före detaljpolering. Annars förbättras enskilda skärmar men inte användarens resa.':'Testing showed that navigation must be solved before polishing details. Otherwise individual screens improve, but the user journey does not.',
  'Ett fiktivt service design case som undersökte hela leveransupplevelsen kring Foodora, inte bara appens gränssnitt.':'A fictional service design case examining Foodora’s entire delivery experience, not only the app interface.','En försening upplevs av kunden som ett enda problem men skapas genom beroenden mellan restaurang, bud, plattform och support.':'Customers experience a delay as one problem, but it is created by dependencies between restaurant, courier, platform and support.','Kundresan kartlades tillsammans med frontstage, backstage, kontaktpunkter och aktörer för att synliggöra var information och ansvar bröts.':'The customer journey was mapped across frontstage, backstage, touchpoints and actors to reveal where information and responsibility broke down.','Lösningsriktningen fokuserade på ärligare tidsuppskattning, tidigare information om förändringar och tydligare ansvar genom hela tjänsten.':'The direction focused on more honest time estimates, earlier information about changes and clearer responsibility throughout the service.','Arbetet resulterade i en service blueprint som kopplade kundens frustration till konkreta processer bakom upplevelsen.':'The work produced a service blueprint connecting customer frustration to concrete processes behind the experience.','En bra digital lösning räcker inte när problemet finns i tjänstesystemet. Gränssnitt och verksamhetsprocess måste utvecklas tillsammans.':'A good digital solution is not enough when the problem lies in the service system. Interface and operational process must evolve together.',
  'Ett analyscase baserat på Google Merchandise Store där GA4 data kombinerades med en kvalitativ UX granskning av köpresan.':'An analytics case based on Google Merchandise Store, combining GA4 data with a qualitative UX review of the purchase journey.','Data visade var användare föll bort men inte varför. Uppgiften var att koppla beteendemönster till konkreta problem i gränssnittet.':'Data showed where users dropped off, but not why. The task was to connect behavioural patterns to concrete interface problems.','Trafik, flöden och konverteringspunkter analyserades i GA4. Därefter granskades motsvarande skärmar heuristiskt för att formulera förklaringar och åtgärder.':'Traffic, flows and conversion points were analysed in GA4. The corresponding screens were then reviewed heuristically to formulate explanations and actions.','Fungerande produktvägar, gästköp, tydligare kostnadsinformation och mindre friktion före produktvisning prioriterades.':'Working product paths, guest checkout, clearer cost information and less friction before product view were prioritised.','Caset mynnade ut i en prioriterad förbättringslista där varje rekommendation hade stöd i både data och UX observation.':'The case resulted in a prioritised improvement list, with every recommendation supported by both data and UX observation.','Kvantitativ data hittar signalen. Kvalitativ UX analys gör signalen begriplig och möjlig att agera på.':'Quantitative data finds the signal. Qualitative UX analysis makes it understandable and actionable.',
  'Ett fiktivt business design case för Iron Dominion Reborn, en communitydriven nytolkning av ett strategispel.':'A fictional business design case for Iron Dominion Reborn, a community-driven reimagining of a strategy game.','Konceptet behövde skapa långsiktigt spelarvärde utan att bygga retention på aggressiv eller orättvis monetisering.':'The concept needed to create long-term player value without relying on aggressive or unfair monetisation for retention.','Spelarnas pains och gains kartlades i Value Proposition Canvas och kopplades till funktioner, affärslogik, budget, risk och möjlig avkastning.':'Player pains and gains were mapped in a Value Proposition Canvas and connected to features, business logic, budget, risk and potential return.','Multiplayer, karteditor och communityinnehåll prioriterades som värdeskapande funktioner. Monetiseringen byggdes kring transparens och frivillighet.':'Multiplayer, a map editor and community content were prioritised as value-creating features. Monetisation was built around transparency and choice.','Resultatet blev en sammanhållen produkt och affärsriktning där communitynytta och ekonomisk bärkraft kunde bedömas tillsammans.':'The result was a coherent product and business direction where community value and financial viability could be assessed together.','Affärsdesign blir trovärdig först när varje intäktsidé kan kopplas till ett tydligt värde för användaren.':'Business design becomes credible when every revenue idea connects to clear user value.',
  'Gränssnittet behövde vara enkelt och lekfullt utan att skapa feltryck, otydliga val eller oavsiktliga prenumerationer.':'The interface needed to be simple and playful without causing accidental taps, unclear choices or unintended subscriptions.','Idéer utvecklades med Crazy 8s, wireframes och en klickbar Figma prototyp. Föräldrar användes som testgrupp för trygghet och begriplighet.':'Ideas were developed through Crazy 8s, wireframes and a clickable Figma prototype. Parents tested safety and clarity.','En huvudhandling fick tydlig visuell prioritet, återkopplingen förenklades och prenumerationsval tonades ned och separerades.':'One primary action received clear visual priority, feedback was simplified and subscription choices were toned down and separated.','Prototypen gav barnet en tydligare väg genom upplevelsen och gjorde kritiska val lättare för föräldern att förstå och kontrollera.':'The prototype gave children a clearer path through the experience and made critical choices easier for parents to understand and control.','Barnvänlig design handlar inte om fler färger. Den kräver hårdare prioritering, tydligare återkoppling och färre konkurrerande val.':'Child-friendly design is not about adding more colours. It requires stricter prioritisation, clearer feedback and fewer competing choices.',
  '04 / MUSIK / UTANFÖR DESIGNEN':'04 / MUSIC / BEYOND DESIGN','05 / KONTAKT':'05 / CONTACT','Research & förståelse':'Research & understanding','Intervjuer, observation, användartest, analys, kundresor och insiktsarbete.':'Interviews, observation, user testing, analysis, customer journeys and insight work.','Informationsarkitektur, wireframes, prototyper, UI och tillgänglighet.':'Information architecture, wireframes, prototypes, UI and accessibility.','Samarbete & riktning':'Collaboration & direction','Facilitering, lagledning, kommunikation och tvärfunktionella team.':'Facilitation, team leadership, communication and cross-functional teams.','Prompting, konceptvisualisering, idéutveckling och snabbare prototyparbete.':'Prompting, concept visualisation, ideation and faster prototyping.','Experience Designer, 2025 till nu':'Experience Designer, 2025 to present','Shift Lead och kundservice, 2019 till 2025':'Shift Lead and customer service, 2019 to 2025','Snickare med känsla för material, människor och hållbara lösningar':'Carpenter with a feel for materials, people and lasting solutions'
});
Object.assign(englishCopy,{
  'Förvandla en tung informationsyta till en relevant lärplattform.':'Turn a dense information surface into a relevant learning platform.','Informationsarkitektur, testobservation och designiteration.':'Information architecture, test observation and design iteration.','En mer personlig, motiverande och överblickbar utbildningsupplevelse.':'A more personal, motivating and accessible learning experience.','Göra ett stort tapetsortiment enklare att förstå, söka och välja ur.':'Make a large wallpaper range easier to understand, search and choose from.','Research, testupplägg, analys och iterativ prototypdesign i Figma.':'Research, test planning, analysis and iterative prototype design in Figma.','Tydligare filtrering, guidning och ett mer tillgängligt köpflöde.':'Clearer filtering, guidance and a more accessible purchase flow.','Kundresa och tjänstesystem.':'Customer journey and service system.','Visualiserade hur problem sprids mellan kund, bud, restaurang och support.':'Visualised how problems spread between customer, courier, restaurant and support.','Kombinerade beteendedata med kvalitativa observationer för bättre beslut.':'Combined behavioural data with qualitative observations for better decisions.','Community, affärsvärde och konceptutveckling.':'Community, business value and concept development.','Kopplade spelarens drivkrafter till en hållbar produktriktning.':'Connected player motivations to a sustainable product direction.','UX Research · Web · Tillgänglighet':'UX Research · Web · Accessibility','UX Research · Web · TillgänglighetSKOLCASE / KONCEPT':'UX Research · Web · Accessibility · Student case / concept','SKOLCASE / KONCEPT':'STUDENT CASE / CONCEPT','Webbplats, självskattning och stöd i vanliga medlemsärenden.':'Website, self-assessment and support for common membership matters.','Tydligare språk, navigering och prioritering utifrån användarnas behov.':'Clearer language, navigation and prioritisation based on user needs.','01 / MÅL':'01 / GOAL','03 / LÄRDOM':'03 / LEARNING','Barnvänlig interaktion och trygg användning.':'Child-friendly interaction and safe use.','Prototyper och tester formade ett enkelt gränssnitt med tydlig återkoppling.':'Prototypes and testing shaped a simple interface with clear feedback.',
  'Aether 2.0 är en spekulativ upplevelse om en framtid där människor kan dela drömmar. Projektet gör en abstrakt idé konkret, lockande och samtidigt obekväm.':'Aether 2.0 is a speculative experience about a future where people can share dreams. The project makes an abstract idea tangible, alluring and unsettling at the same time.','01 / FRÅGAN':'01 / THE QUESTION','Vad händer när det mest privata blir gemensamt?':'What happens when our most private space becomes shared?','Vi utforskade hur teknik kan skapa närhet men också nya former av kontroll och utsatthet.':'We explored how technology can create closeness, but also new forms of control and vulnerability.','Från utopi till kritisk upplevelse.':'From utopia to critical experience.','Research, framtidsscenarier, visuell prototyping, AI verktyg, ljud och film formade berättelsen.':'Research, future scenarios, visual prototyping, AI tools, sound and film shaped the narrative.','Koncept, gestaltning och sammanhang.':'Concept, expression and context.','Jag arbetade med idéutveckling, visuellt språk, prompting och hur delarna skulle bli en helhet.':'I worked on concept development, visual language, prompting and shaping the parts into a whole.','Ett sammanhållet framtidskoncept som kombinerar berättande, kritisk design och visuell upplevelse.':'A cohesive future concept combining storytelling, critical design and visual experience.','Det starkaste konceptet visar både löftet och priset i den framtid det föreslår.':'The strongest concept reveals both the promise and the cost of the future it proposes.',
  'Ett fiktivt experience design case som betraktade biobesöket som en hel resa, från första filmsuget till vägen hem.':'A fictional experience design case viewing the cinema visit as a complete journey, from the first desire to see a film to the journey home.','Själva filmen är bara en del av upplevelsen. Planering, sällskap, transport, väntan och efterkänsla påverkar helhetsvärdet.':'The film itself is only part of the experience. Planning, company, transport, waiting and the feeling afterwards all shape its overall value.','Hypoteser och research sammanställdes i en customer journey med sex steg, där handlingar, kontaktpunkter, förväntningar och känslor följdes över tid.':'Hypotheses and research were synthesised into a six-stage customer journey tracking actions, touchpoints, expectations and emotions over time.','Möjligheter identifierades före, under och efter visningen i stället för att begränsa lösningsrymden till biosalongen.':'Opportunities were identified before, during and after the screening instead of limiting the solution space to the auditorium.','Kartläggningen skapade en gemensam bild av upplevelsen och visade var tjänsten kan minska friktion eller förstärka förväntan.':'The map created a shared view of the experience and showed where the service could reduce friction or build anticipation.','Experience design kräver att man ritar en större ram än den produkt eller plats som först verkar vara uppdraget.':'Experience design requires framing more broadly than the product or place that first appears to define the brief.','Den sammanhängande bioupplevelsen.':'The connected cinema experience.','Kartlade hela resan och möjligheter före, under och efter besöket.':'Mapped the entire journey and opportunities before, during and after the visit.',
  'Ett fiktivt user research case om bilisters upplevelse av att hitta, förstå och planera parkering i Stockholm.':'A fictional user research case about drivers’ experience of finding, understanding and planning parking in Stockholm.','Ämnet rymmer starka antaganden. Utmaningen var att förstå faktisk frustration, osäkerhet och beteenden innan en lösning valdes.':'The topic carried strong assumptions. The challenge was to understand real frustration, uncertainty and behaviour before choosing a solution.','Researchfrågan avgränsades, intervjufrågor formulerades och svaren syntetiserades i återkommande behov och mönster.':'The research question was scoped, interview questions were formulated and responses synthesised into recurring needs and patterns.','Fokus låg på förutsägbarhet, begriplig information och planering snarare än att omedelbart föreslå ännu en parkeringsapp.':'The focus was predictability, understandable information and planning rather than immediately proposing another parking app.','Arbetet gav en behovsbild som kan användas för att prioritera framtida koncept och testa rätt problem före utveckling.':'The work produced a needs map that can guide future concepts and validate the right problem before development.','Bra research håller lösningen öppen tillräckligt länge för att användarens problem ska hinna förändra riktningen.':'Good research keeps the solution open long enough for the user’s problem to change the direction.','Intervjuer och behovskartläggning.':'Interviews and needs mapping.','Synliggjorde frustration, osäkerhet och behov av bättre förutsägbarhet.':'Revealed frustration, uncertainty and a need for greater predictability.',
  'Ett fiktivt frontend och UI case där en portfolio utvecklades från wireframe i Figma till en fungerande responsiv webbplats.':'A fictional frontend and UI case where a portfolio evolved from a Figma wireframe into a working responsive website.','Innehållet behövde vara snabbt att bedöma för en arbetsgivare och samtidigt fungera på olika skärmstorlekar och i riktig kod.':'The content needed to be quick for an employer to assess while working across screen sizes and in real code.','Informationshierarkin definierades i wireframes, översattes till HTML och CSS och testades löpande mellan desktop och mobil.':'The information hierarchy was defined in wireframes, translated into HTML and CSS and continuously tested across desktop and mobile.','Projekt, kompetenser och kontakt fick tydlig ordning. Responsiva regler utgick från innehållets läsbarhet i stället för enskilda enheter.':'Projects, skills and contact information gained a clear order. Responsive rules were based on content readability rather than specific devices.','Resultatet blev en kodad webbupplevelse som förenade visuell identitet, tydlig navigation och praktisk responsivitet.':'The result was a coded web experience combining visual identity, clear navigation and practical responsiveness.','När designen byggs i kod blir begränsningar synliga tidigare och besluten kan bedömas i sitt verkliga medium.':'Building the design in code reveals constraints earlier and lets decisions be assessed in their real medium.','Responsiv UI och frontend.':'Responsive UI and frontend.','Omsatte en visuell identitet till en fungerande webbupplevelse.':'Translated a visual identity into a working web experience.',
  'Ett fiktivt strategi och konceptcase för Worldline Media Group med fokus på relevans hos en yngre, digital målgrupp.':'A fictional strategy and concept case for Worldline Media Group focused on relevance for a younger digital audience.','Ett värderingsdrivet mediehus behövde hitta nya uttryck och deltagandeformat utan att förlora sin trovärdighet.':'A values-driven media company needed new forms of expression and participation without losing credibility.','Målgruppens medievanor undersöktes och översattes till konceptmöjligheter. Arbetet strukturerades med sprintar, pipeline, tidsplan, budget och risk.':'The audience’s media habits were studied and translated into concept opportunities. The work was structured through sprints, a pipeline, timeline, budget and risk.','Kortformat, interaktion och medskapande prioriterades framför envägskommunikation. Konceptets genomförbarhet prövades parallellt.':'Short-form content, interaction and co-creation were prioritised over one-way communication. Feasibility was evaluated in parallel.','Resultatet blev en strategisk konceptriktning som kopplade målgruppsrelevans till produktion, resurser och möjliga affärseffekter.':'The result was a strategic concept direction linking audience relevance to production, resources and potential business impact.','Ett framtidskoncept blir användbart först när berättelsen också har en realistisk väg till genomförande.':'A future concept becomes useful only when its story also has a realistic path to delivery.','Research, målgrupp och framtida relevans.':'Research, audience and future relevance.','Översatte ungas medievanor till strategiska konceptmöjligheter.':'Translated young people’s media habits into strategic concept opportunities.'
});
...
const accessibleEnglish={
  ...
  'Konceptbild från AcadeMedia Academy':'Concept image from AcadeMedia Academy',
  'Designbeslut för struktur, motivation och friare konceptarbete':'Design decisions for structure, motivation and freer concept work',
  'Från identifierade brister till en förfinad och tillgängligare upplevelse':'From identified issues to a refined and more accessible experience',
  'Aethers produktionsprocess från skiss till efterbearbetning':'Aether’s production process from sketch to post-production',
  'Aethers utveckling från utopisk vision till sårbarhetsanalys':'Aether’s development from utopian vision to vulnerability analysis',
  'Konceptbild från Duro 2.0':'Concept image from Duro 2.0',
  'Tre omgångar från bred research till teknisk verifiering':'Three rounds from broad research to technical verification',
  'Insikter som förändrade sök, filtrering och guider':'Insights that changed search, filtering and guides',
  'Grannfejdens startskärm med kvarteret Björkbacken och lobbyval':'Grannfejden start screen showing the Björkbacken neighbourhood and lobby options',
  'Kortval i Grannfejden med resurser, handlingar och guide':'Card selection in Grannfejden with resources, actions and guidance',
  'Grannfejdens snabbguide för spelets tre huvudmoment':'Grannfejden quick guide to the game’s three main stages',
  'Roterande kodbyggd 3D-modell av ett hönshus':'Rotating code-built 3D model of a chicken coop',
  'Konceptbild från Kommunal':'Concept image from Kommunal',
  'Om MYKO-projektet':'About the MYKO project'
};
function setPortfolioLanguage(language){
  ...
  const kommunalMeta=document.querySelector('.project[href="#kommunal"] .project-title p');
  if(kommunalMeta?.firstChild)kommunalMeta.firstChild.nodeValue=language==='en'?'UX Research · Web · Accessibility':'UX Research · Web · Tillgänglighet';
  document.querySelectorAll('.project-kind').forEach(kind=>{
    const source=originalText.get(kind.firstChild)?.trim()||kind.textContent.trim();
    kind.textContent=language==='en'?(englishCopy[source]||source):source;
  });
